...
{
    unsigned code = 0xE0;
    std::ifstream in("./roms/pong.ch8", std::ios::in | std::ios::binary | std::ios::ate);
    std::ifstream::pos_type size;
    char* romData;
    if (!in.is_open()) {
        std::cerr << "'. /roms/pong.ch8' 파일 열기를 실패했습니다." << std::endl;
    }
